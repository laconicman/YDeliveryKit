...
nonisolated extension RoutePoint {
    ...
    ///
    /// The rule is deliberately narrow: a leading segment is shed only when it
    /// carries no digits (a house number does; a city never does) and at least
    /// two segments remain — anything else returns the address verbatim.
